...
export default function HomeScreen({ navigation }: Props) {
  ...
  const updateTodayEntries = useCallback(
    (allEntries: Entry[]) => {
      ...
      console.log(
        `Updating today entries: ${todaysEntries.length} entries for today`,
      );
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        console.log(
          `HomeScreen: Initial load - ${allEntries.length} entries from storage`,
        );
    ...
    useCallback(() => {
      ...
      const load = async () => {
        try {
          ...
          const allEntries = await loadEntries();
          console.log(
            `HomeScreen (focus): Loaded ${allEntries.length} entries from storage`,
          );
          ...
          if (!isInitialDataLoadedRef.current) {
            isInitialDataLoadedRef.current = true;
            console.log(
              "HomeScreen: Initial data loaded via focus effect, persistence enabled",
            );
          }
  ...
  useEffect(() => {
    const persistEntries = async () => {
      ...
      try {
        console.log(
          `HomeScreen: Saving ${entries.length} entries (data changed)`,
        );
  ...
  const handleAIEstimate = async () => {
    if (!label.trim()) {
      Alert.alert(
        ...
        "AI needs a description to provide nutrition estimates.",
      );
      return;
    }
    ...
    try {
      ...
      if (result) {
        ...
        setErrorMessage(
          "Unable to get nutrition estimates. Please try again with a more detailed description.",
        );
      }
    } catch (error) {
      console.error("Error estimating nutrition:", error);
      setErrorMessage(
        "Something went wrong trying to estimate nutrition values.",
      );
    } finally {
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      if (transcript) {
        ...
        setErrorMessage(
          "Unable to transcribe your voice. Please try again or enter your meal manually.",
        );
      }
    } catch (error) {
                    ...
                    <View style={styles.largeInputContainer}>
                      <TouchableWithoutFeedback onPress={dismissKeyboard}>
                        <View
                          style={[
                            styles.largeInput,
                            isTranscribing && styles.disabledInput,
                          ]}
                        >
                          {isTranscribing ? (
